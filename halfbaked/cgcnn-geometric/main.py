...
def train(model, device, train_loader, loss_criterion, accuracy_criterion, optimizer, epoch, normalizer):
...
def validate(model, device, test_loader, loss_criterion, accuracy_criterion, normalizer):
    batch_time = AverageMeter('Batch', ':.4f')
    losses = AverageMeter('Loss', ':.4f')
    accus = AverageMeter('Accu', ':.4f')
    progress = ProgressMeter(
...
def main():
    ...
    data0 = dataset[0]
    model = CGCNN(data0.x.size(-1), data0.edge_attr.size(-1))
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        train_loss, train_accu = train(model, device, train_loader, loss_criterion, accuracy_criterion, optimizer,
                                       epoch, normalizer)
        val_accu = validate(model, device, val_loader, loss_criterion, accuracy_criterion, normalizer)
        # scheduler.step()
