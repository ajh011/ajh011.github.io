...
\subsection*{MAC Addresses for Static IP Addresses}
...
HeadNode & a8:a1:59:c7:0a:34 & 129.10.15.189/50.159\\
...
The construction of a new SLURM computational cluster consists of three basic steps: \textit{(1)} constructing a Local Access Network (LAN) and mounting the Network Attached Storage (NAS) across it; \textit{(2)} homogenizing users and software across nodes; and, \textit{(3)} installing SLURM and initializing it's daemons. Each of these basic steps are detailed in the following sections.
...
\end{verbatim} 
Note that these commands require that the device (ethernet port) labeled enp36s0f0 is outward-facing (to the internet) and the device labeled enp36s0f1 is inward-facing.
...
\end{verbatim}
See \url{https://askubuntu.com/questions/66890/how-can-i-make-a-specific-set-of-iptables-rules-permanent}.
...
\end{verbatim}
To set a static DNS server on Ubuntu that doesn't get reset on restart, first install \verb|resolveconf| with \verb|apt|. Then, append a line containing the desired DNS server IP address after `nameserver' in  \verb|/etc/resolv.conf| or \verb|/etc/resolvconf/resolv.conf.d/head| and then run \verb|sudo service resolvconf restart|.  For more detail, see \url{https://datawookie.dev/blog/2018/10/dns-on-ubuntu/} or \url{https://anyware.hp.com/web-help/cas_manager/22.04/troubleshooting/dns_name_resolution/}. Note that 8.8.8.8 is Google's publicly available DNS server, and thus is a good choice due to it's reliability.
...
\subsubsection{Logging Into Subnode Via SSH Jumping}
...
\subsubsection{Generating SSH Keys For Automatic (Password-less) Login}\label{savesshkeys}
...
\subsubsection{Cluster-wide Playbooks}
Ansible runs \textit{playbooks}, specified in YAML files, across sets of nodes.
...
\begin{verbatim}
	...
	- python
	- conda
\end{verbatim}See \url{https://opensource.com/article/20/9/install-packages-ansible}.
...
\end{verbatim}
For generic shell commands across the cluster using ansible, see \url{https://kodekloud.com/blog/ansible-shell-module/}.
...
\subsection{Submitting Jobs (SLURM)}
SLURM jobs may be run across the cluster by submitting \verb|sbatch| commands.


\subsection{Cluster-wide Commands \& Installation (Ansible)}
