...
\begin{abstract}
Modern E(3)-equivariant networks have enabled accurate and efficient predictions of rotationally equivariant properties beyond scalar quantities. Tensorial quantities of crystalline systems, such as dielectric, piezoelectric, and elasticity tensors, are highly correlated with functionalities of complex materials but computationally expensive to produce \textit{ab initio}. In this work, we demonstrate a comprehensive framework for building equivariant network architectures for tensorial property predictions by explicitly developing spherical harmonic decompositions of second- to fouth-order tensorial properties using their general symmetries. Three distinct E(3)-equivariant convolutional structures are developed and applied to predict the components of these decompositions, allowing us to predict the aforementioned tensorial quantities in an equivariant manner and compare the model performance. The models are trained using a large set of existing data in public material databases. Model performance for different tensorial components and the role of symmetry constraints are discussed. We further report the transferability of these predictive models between different tensorial target sets. The work presents a fundamental step toward the effective use of modern equivariant networks for the discovery and design of functional materials.
\end{abstract}
\vspace{.5cm}

\section{Introduction}
Traditional machine learning methods applied to the material sciences have focused on the prediction of invariant, scalar properties of material systems\cite{mlreview1, mlreview1.5, mlreview2, mlreview2.25, mlreview2.5, mlreview3}. The development of equivariant models enables the prediction of coordinate-system-dependent outputs in a well-defined manner \cite{tensorfieldnetworks, e3nn_paper}. Some applications of these equivariant networks neglect the direct prediction of directional (i.e. coordinate system-dependent) quantities and instead are used to predict invariant quantities to greater accuracy, from which directional quantities are then often derived \cite{nequip, magnet}. Other applications have utilized the full capabilities of equivariant models to predict outputs associated with spherical harmonics \cite{deeph-e3, xpainn}, or use equivariant outputs to predict tensorial quantities of solid-state materials \cite{tensornet, painn, etgnn, yan2024equivcrystalprediction, MatTen, StrainNet, piezo-npj}. However, these tensorial prediction works often give very brief descriptions of the harmonics decompositions used.

%Other works, such as StrainNet \cite{StrainNet} have previously used equivariant graph transformers and additional information about the strained structure of a crystal to predict elasticity tensors. MatTen \cite{MatTen} utilized a steerable equivariant convolutional neural network to predict the elastic tensor with a decomposition based on the same principles demonstrated here. 

In rotationally equivariant models, features are often associated with irreducible representations of the rotation group \cite{e3nn_paper, tensorfieldnetworks}, generally referred to as spherical harmonics. This association may be leveraged to predict tensorial quantities directly from the outputs of such models. This component-wise prediction of tensorial properties is achieved by decomposing tensors into harmonic subspaces via spherical harmonic decomposition, by which we may also associate arbitrary tensors with the irreducible representations of the rotation group \cite{harmonictensors}. This allows one to essentially read off tensors component-wise from the output representations of these equivariant models by matching rotational and azimuthal indices.

Here,  we give explicit harmonic decompositions of three material property tensors: the dielectric tensor; the piezoelectric tensor; and the elasticity tensor, following the approaches given in \cite{itin-elastic, itin-rank3}. %(with a brief overview of the mathematics involved in the Supplementary Note S1) 
Results are then presented for for the prediction of these material property tensors directly from crystalline structures, as well as pretraining sequences composed of different scalar and tensorial targets investigating the transferability of such models between tasks. The structure of this work is as follows: \cref{eqnet} gives a brief overview of the components of modern rotation-equivariant networks; \cref{so3inv} details the harmonic decomposition of tensor spaces, drawing an analogy to feature spaces of equivariant models. The explicit decomposition of three common material property tensors is then given in \cref{briefdecomps}. Section \ref{methods} gives an overview of the basic model architecture that may be used to predict such tensors, as well as the specific implementations used here; and \cref{results} presents the prediction accuracy of such models in the prediction of the aforementioned material property tensors, as well as results detailing a set of pretraining chains between different target sets.



\section{Rotationally Equivariant Neural Networks}\label{eqnet}
...
A specific class of neural networks, referred to as Graph Neural Networks (GNN) \cite{cgcnn,alignn,amdnet,chemgnn,geocgcnn,icgcnn} or, more generally, message passing networks (MPNN) \cite{mpnn}, act on feature vectors $v_n^L$, which are associated with nodes $n$ in a representative graph (i.e. a collection of nodes and connections between them) for each layer $L$ of the network. These models preserve the underlying connectivity of the graph representation, and act in a way that is invariant under node-index permutation.% and, with appropriate choice of initial node features, maintain coordinate-system invariance.
%Such graph networks are also often rotationally invariant, however. 

MPNNs are generally invariant under translations as long as only coordinate-system invariant features (e.g. interatomic distance) are included. As such, these models are incapable of predicting any coordinate-system quantities, such as vector or tensor components. 
In material systems, we generally desire representations that are equivariant under the group of transformations acting on real space. This corresponds to the special Euclidean group $SE(3)$, which consists of translations of the origin and the group of proper rotations $SO(3)$ \cite{e3nn_paper}. 

Equivariant functions are functions that preserve the action of a group on it's domain in it's codomain. For a function $f:X\rightarrow Y$, where $X$ and $Y$ are vector spaces with some set of group representations $\mathcal{D}^{X}(G)$ and $\mathcal{D}^Y(G)$ acting on these spaces respectively, $f$ is equivariant with respect to $G$ if it satisfies:
$$
\mathcal{D}^{Y}(g)f(x)=f\big( \mathcal{D}^{X}(g)x\big)
$$
Compositions of equivariant functions are themselves equivariant functions  \cite{equivariant_cohen}. As such, we may form an equivariant network by composing it layer-wise from a set of common equivariant functions. Here, we consider four types of $SO(3)$-equivariant functions from which we may compose our equivariant networks layer-wise \cite{equivariant_cohen}. These consist of: $SO(3)$-feature convolutions, $\ell$-wise self-interactions and non-linearities, and pooling across nodes. A brief description of each is given in Supplementary Note \ref{eq_layers}.
...
In such cases, the node-wise features $(v_{nc}^L)_{\ell}^m$ of channel $c$, are equipped with an additional two indices $\ell$ and $m$ \cite{tensorfieldnetworks}, corresponding to the aformentioned indices of the harmonic representations.
...
The outputs of these networks are then also associated with some indices $\ell$ and $m$, which may be interpreted as a set of spherical harmonic basis components \cite{e3nn}.
...
We may construct a set of spherical harmonic tensors $\hat{Y}_{\ell}^m$ by first defining the $J_z$ vector basis and then considering symmetric products of such vectors. We define the $J_z$ basis \cite{mochizuki1988spherical, mochizuki1996spherical, edmonds}:
$$
\begin{bmatrix}
a_+ \\
a_0 \\
...
\end{bmatrix}
$$
where $x,y,z$ are Cartesian components of the same vector. In this basis, the unit vectors associated with these components correspond to unit vector spherical harmonics $\hat{Y}_{\ell=1}^m$ (where we will assume Racah normalization for all definitions\footnote{In this work, we define the spherical harmonic functions in the Racah normalization as: $Y_{\ell}^m(\theta, \phi)=\sqrt{\frac{(\ell-m)!}{(\ell+m!)}}P_{\ell}^m(\cos\theta)e^{im\phi}$; which makes Clebsch-Gordon relations simpler.}).
Products of spherical harmonics then may be decomposed into sums of single spherical harmonics by way of the Clebsch-Gordon expansion \cite{sakurai}:
$$
...
$$
where $c_{\ell_1 m_1 \ell_2 m_2}^{LM}$ are the Clebsch-Gordon coefficients.
This expansion may be used to draw a correspondence between symmetric tensor products of $n$ vectors in the $J_z$ basis and order $\leq n$ spherical harmonic tensors. Thus giving us a sets of basis tensors for symmetric tensor spaces that transform as irreducible representations of $SO(3)$. The coefficients for such a transformation relevant for tensors describing the material properties considered here are listed in \cref{cgcoef}.
...
Since the basis provided by the construction method above only applies to symmetric tensor spaces, tensors of arbitrary symmetry may need to first be decomposed into a set of totally symmetric subspaces. We may decompose an arbitrary tensor of order-$n$ into a set of symmetric tensors of order-$\ell$ with $0\leq\ell\leq n$ by projecting it onto it's irreducible $SO(3)$-invariant subspaces. We then may use the relations between the $J_z$ basis components and the real spherical harmonic components $y_{\ell m}$ using the relations discussed above.
...
As a basic example, consider a general order-two tensor, as displayed in \cref{rank2example}, where we may first construct the totally symmetric and antisymmetric subspaces, and then contract with the metric tensor $g_{ij}$ and the totally antisymmetric Levi-Civita tensor $\epsilon_{ij}$, respectively. This achieves an irreducible $SO(3)$ decomposition of the space into a scalar (order-zero) trace $s$, a symmetric order-two residue $R$, and a scalar antisymmetric component $a$. 



%consider a order-two tensor, we may always decompose it with respect to GL first (by using the corresponding Young symmetrizers), resulting in a symmetric tensor $S$ and antisymmetric tensor $A$.
%The symmetric $S$ then has an invariant order-0 space, commonly referred to as it's trace, and constructed via total contraction with the metric tensor $g_{ij}$. The traceless residue of $S$ then is a symmetric, order-2 tensor which is itself an invariant subspace. The antisymmetric $A$ then may be fully contracted with the totally antisymmetric $\epsilon_{ij}$ to return another order-0 invariant subspace.

\begin{figure*}[t]
\begin{center}

\includegraphics[scale=.39]{bigfig.pdf}
\end{center}
...
\section{Spherical Harmonic Decomposition of Common Material Property Tensors}\label{briefdecomps}

 Here is presented a brief overview of the harmonic decomposition of three common, tensor-valued material properties: the dielectric tensor $\epsilon$, the piezoelectric strain tensor $d$, and the elasticity tensor $C$. A more systematic presentation of each decomposition is given in reference \cref{decomps}. Note that in the following, we ignore the distinction between co- and contravariant components due to the Euclidean structure $g_{ij}=\delta_{ij}$.
...
$$
Here we focus only on static dielectric responses, that is, $(\partial/\partial t)\vec{E}=0$, primarily due to the restriction of data to this case. 
The dielectric tensor $\epsilon$ is an order-two tensor formed of vector spaces over the field $\mathbb{R}^3$.
...
$$
resulting in two invariant subspaces of order-0 and 2, respectively. These two symmetric subspaces then admit a decomposition in the manner described in \cref{so3inv}.
...
The mixed symmetry part $A$ also requires decomposition with respect to $SO(3)$, so that we have a set of symmetric tensors describing it. It's 8 independent components can be described by a $5\oplus 3$ dimensional space consisting of a symmetric order-2 tensor and a trace vector.
...
The order-2 invariant subspace of $A$ then may be constructed by symmetrizing the partial contraction with $\epsilon$ along the first and third indices (the anti-symmetric pair). Note that the antisymmetric part of this partial contraction corresponds to the trace vector space accounted for here by $u$. Explicitly, we define:
$$
b_{ij}=\frac{1}{2}\big(\epsilon_{nim}A_{njm}+\epsilon_{njm}A_{nim}\big)
$$
which is a traceless symmetric order-2 tensor.

And so, as mentioned above, the symmetric part $S$ is readily decomposed in the spherical bases into $\mathcal{H}^{(1)}\oplus \mathcal{H}^{(3)}$. The mixed-symmetry part then inhabits the space $\mathcal{H}^{(1)}\oplus \mathcal{H}^{(2)}$.
...
$$
As should be clear by the number of indices, the elastic tensor is a fourth order tensor. 
...
$$
This tensor $t$ then has a harmonic decomposition according to the order-two Clebsch-Gordon transformation between the $J_z$ basis and the harmonic basis $y_{\ell}^m$. 
...
The data utilized is available in the \textit{Materials Project} database \cite{matproj} which includes $\sim$150k materials. Of which, almost all have calculated scalar values for band gap and formation energy, $\sim$12k have a calculated elastic tensor \cite{elastic-matproj}, $\sim$6k have a calculated  dielectric tensor \cite{di-matproj}, and $\sim$3k have a calculated piezoelectric tensor \cite{piezo-matproj}. The data was split  70\% for training and 15\% for validation, with a remaining 15\% held out for testing. Data distribution statistics can be found in \cref{data-dist}.
...
\begin{figure*}[!h]\label{best-heatmap}
\begin{center}

\includegraphics[width=0.49\linewidth]{segnn_default_dielectric_heatmap.pdf}
\includegraphics[width=0.5\linewidth]{segnn_default_piezo_heatmap.pdf}
\includegraphics[width=0.6\linewidth]{segnn_default_elastic_heatmap.pdf}
\end{center}
...
\section{Conclusion}
As outlined in this work, $SO(3)$ equivariant networks may be used to directly predict tensor components in a manner consistent with differing choices of basis for input features due to the equivariance of the network.
Results presented here focus on three common tensorial properties of solid state materials describing dielectric, pieozoelectric and elastic responses. However, the approach taken here may be applied to arbitrary tensorial targets by first decomposing into their $GL$-invariant subspaces by way of Young symmetrizers, then their $SO(3)$-invariant subspaces and finally transforming these symmetric tensor subspaces into a spherical tensor basis.
...
For example, for a order-three tensor $T$, the volume may be defined component-wise as:
$$
...
$$
Furthermore, we may construct different invariants for different order tensor spaces. For example, we may define $O$ invariant scalar-valued traces  $Tr(\cdot)$ of second order tensors $M$ by means of the metric tensor, defined component wise as
$$
Tr(M) = g_{ij}M^{ij}
$$
and we may form three $O$ invariant trace vectors for third order tensors $T$, defined component wise as:
$$
\nu^{k} = g_{ij}T^{ijk},\quad \mu^{j} = g_{ki}T^{ijk},\quad u^{i} = g_{jk}T^{ijk}
$$
That is, with the metric tensor $g_{ij}$ available, as is the case in $SO(3)\subset O(3)$, we may decompose a order-$n$ tensor into a sets of order-$(n-2),(n-4),...,(0$ or $1)$ tensors by taking sequential traces.
...
In $SO(3)$, we have all of the above tools (Young diagrams, $g_{ij}$, and $\epsilon_{ijk}$) available. As such, we may decompose a tensor into a set of $SO(3)$ invariant subspaces by their use, and all will be valid. Unfortunately, for tensors of order$>2$, there is not, in general, a unique and irreducible set of invariant $SO(3)$ spaces. However, we may always decompose an arbitrary tensor into a set of (not necessarily unique) irreducible $SO(3)$-invariant symmetric tensor subspaces using the tools given above.



\section{Full Decompositions of Tensors}\label{decomps}
Below, we give the $SO(3)$ decomposition of the three tensors of interest here. Namely, the order-two dielectric tensor $\epsilon$, the order-three piezoelecrtic strain tensor $d$, and the order-four elastic tensor $C$. Each begins with the Young diagrams for the total order tensor space and then uses the symmetries of the respective tensor to restrict down to the relevant Young tableaux. These are used to form symmetrizers and then further decomposed with contractions with the metric tensor $g$ and the levi-civita tensor $\epsilon$ until all that remains are a set of fully symmetric sub-tensors.
...
The GL decomposition of a order-two tensor space is the usual symmetric-antisymmetric decomposition of a matrix $M$.
$$
...
\end{center}
We can then further decompose this order-two symmetric into a order-zero space and a order-two space by taking the order-0 trace $t = g_{ij}\epsilon_{ij}$ and then forming the order-2 traceless residue $R_{ij}=\epsilon_{ij}-t\delta_{ij}$.
\subsection{Piezoelectric Tensor Decomposition}
The Young diagrams for a order-three tensor are as follows:
\begin{center}
...
The mixed symmetry part $A$ however requires further decomposition with respect to $SO(3)$, so that we have a set of symmetric tensors describing it. It's 8 independent components can be described by a $5\oplus 3$ dimensional space consisting of a symmetric order-2 tensor and a trace vector.
...
Rather simply then, we can reconstruct a third-order tensor $V$, corresponding to this order-one invariant subspace, by defining $V$ component-wise as:
$$
V_{ijk} =\frac{1}{4}\big[v_i g_{jk}+ v_j g_{ik} - 2 v_j g_{ik}\big]
$$
The order-2 invariant subspace of $A$ then may be constructed by symmetrizing the partial contraction with $\epsilon$ along the first and second indices (the anti-symmetric pair). Note that the antisymmetric part of this partial contraction corresponds to the trace vector space accounted for here by $u$. Explicitly, we define:
$$
b_{ij}=\frac{1}{2}\big(\epsilon_{i}^{mk}A_{mkj}+\epsilon_{j}^{mk}A_{mki}\big)
$$
which is a traceless symmetric order-2 tensor. And from which we may reconstruct the corresponding order-3 tensor $B$, defined by:
$$
B_{ijk}=\frac{1}{3}\big[ \epsilon_{ik}^p b_{pj}+\epsilon_{ij}^p b_{pk}\big]
$$
Thus, we may provide a total harmonic decomposition for $A$ in terms of the invariant subspaces of the order-1 $u$ and the order-2 $b$, such that:
$$
...
If we examine the Young diagrams of a fourth order tensor, as below: 
...
\section{$SO(3)$ Equivariant Layers}\label{eq_layers}
Since compositions of equivariant functions are themselves equivariant, we need only consider a set of basic equivariant functions which may then be composed layer-wise to construct arbitrarily large equivariant networks. Below, we give brief descriptions of four common equivariant functions \cite{equivariant_cohen} used in such networks, namely: point-wise nonlinearities; self-interaction; pooling functions; and the heart of equivariant networks, equivariant convolution.
...
\section{Formulas for Scalar Elastic Properties from Elastic Tensor}\label{scalar-elastic-formulas}
\begin{equation*}
...
\end{tabular}
\caption{Coefficients for transformation between spherical basis components $a_{\alpha\beta\gamma\delta}$ and harmonic components $y^m_l$ for a order-2,3,4 symmetric tensor. Note that the order-4 is only relevant for symmetric components of elastic tensors and correspond to a transformation \cite{mochizuki1988spherical} between the $J_z$ basis and their symmetric tensor $s$.}
\end{table*}
...
\section{Symmetry Classes of Tensors}

\subsection{Dielectric Tensors}
The symmetry and number of independent components of dielectric tensors are determined by the crystal class of the associated structure.
\begin{align*}
    \mathbf{Triclinic}: \begin{bmatrix}
        a & d & e \\
         & b &f\\
         *& & c
    \end{bmatrix}\\
    \mathbf{Monoclinic}: \begin{bmatrix}
        a & 0 & d \\
         & b &0\\
         *& & c
    \end{bmatrix}\\
    \mathbf{Orthorhombic}: \begin{bmatrix}
        a & 0 & 0 \\
         & b &0\\
         *& & c
    \end{bmatrix}\\
    ...
    \end{array}
     \begin{bmatrix}
         ...
         *& & b
    \end{bmatrix}\\
    \mathbf{Cubic}: \begin{bmatrix}
        a & 0 & 0 \\
         & a &0\\
         *& & a
    \end{bmatrix}\\
\end{align*}
\subsection*{Component-wise MAE by Crystal Class}

\includegraphics[scale=0.5]{sym_heatmaps/dielectric/triclinic.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/dielectric/monoclinic.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/dielectric/orthorhombic.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/dielectric/tetragonal.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/dielectric/cubic.pdf}

\subsection{Piezoelectric Tensors}
The symmetry and number of independent components of piezoelectric tensors are determined by the point group of the associated structure.
\subsection*{Component-wise MAE by Point Group}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/1.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/2.pdf}


\includegraphics[scale=0.5]{sym_heatmaps/piezo/m.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/222.pdf}


\includegraphics[scale=0.5]{sym_heatmaps/piezo/mm2.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/3.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/32.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/3m.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/6_4.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/6mm_4mm.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/6b.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/6bm2.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/4b.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/piezo/4b2m.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/piezo/23_4b3m.pdf}


\subsection{Elastic Tensors}
The symmetry and number of independent components of elastic tensors are determined by the crystal class of the associated structure.
\subsection*{Component-wise MAE by Crystal Class}

\includegraphics[scale=0.5]{sym_heatmaps/elastic/triclinic.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/elastic/monoclinic.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/elastic/orthorhombic.pdf}
\includegraphics[scale=0.5]{sym_heatmaps/elastic/tetragonal.pdf}

\includegraphics[scale=0.5]{sym_heatmaps/elastic/cubic.pdf}
