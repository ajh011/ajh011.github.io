...
\begin{frame}{Equivariance}
	A function $f:\mathcal{X}\rightarrow\mathcal{Y}$ between vector spaces is considered equivariant with respect to some group $G$ if for all elements $g\in G$ the following diagram commutes:
	
	\vspace{5cm}
	
	for linear representations $D$ acting on the function's domain and codomain.
...
$$
This can be accomplished by associating hidden features $V$  and filters $F$ with spherical harmonics defining convolution as their tensor product [3]:
$$
...
\begin{frame}{Dielectric Tensor}
	...
	\vspace{0.8cm}
		...
		The piezoelectric strain components $d_{ijk}$ are symmetric under $i,j$ due to the symmetry of the strain tensor $\epsilon_{ij}$.
	
\end{frame}
\begin{frame}{Piezoelectric Tensor}
	The piezoelectric strain constants $(d_{ijk})_{T}$ are defined by:
	$$
	...
	$$
	and where $D$ is the resulting electric displacement vector in the material.

\end{frame}

\begin{frame}{Elastic Tensors}

The elasticity (or simply, elastic) tensor $C$ of a material relates it's Cauchy strain $\epsilon$ to some infinitesimal stress $\tau$. This may be described, component-wise, as the linear relation below:
$$
\tau_{ij}=C_{ijkl}\epsilon_{kl}
$$
For all material systems, $C$ must satisfy the so-called 'minor symmetries' below:
$$
C_{ijkl}=C_{jikl}
$$
$$
C_{ijkl}=C_{ijlk}
$$
For conservative systems, $C$ has the additional 'major symmetry':
$$
C_{ijkl}=C_{klij}
$$
\end{frame}
\begin{frame}{Materials Project Database}
The Materials Project [5] provides calculated scalar and tensorial properties for a large set of ~150,000 materials. After pruning, we are left with 7,273 dielectric tensors, 3,292 piezoelectric tensors, and 10,286 elastic tensors.
\end{frame}
...
\begin{frame}{Prediction Results}
	Elastic Tensor [\textit{GPa}]
	
	\medskip
	
	Piezoelectric Tensor [\textit{C/m$^2$}]
...
[1] Phys. Rev. Lett. \textbf{120}, 145301 (2018)

[2] \textit{https://arxiv.org/abs/1704.01212}

[3] \textit{https://arxiv.org/abs/1802.08219}

[4] \textit{https://arxiv.org/abs/2406.03563}

[5] \textit{https://materialsproject.org/}
